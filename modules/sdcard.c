...
STATIC void sdcard_init_v1(sdcard_SDObject_obj_t *self) {
    for (int i=0; i<CMD_TIMEOUT; i++){
        ...
        if (sdcard_cmd(self, CMD41) == 0){
            self->cdv  = BLOCK;
            self->type = (const char*)"SDCard_v1";
            return;
        }
    }
...
STATIC void SDCard_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind) {
    ...
    mp_printf(print, "SDCard(drive: %s, size mb: %u, type: %s)\n", self->drive, self->sdobject->sectors/2048, self->sdobject->type);
}
...
STATIC mp_obj_t SDCard_detect(mp_uint_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    enum {ARG_self, ARG_automount, ARG_wait, ARG_maxwait, ARG_interval};
    static const mp_arg_t allowed_args[] = {
        ...
        { MP_QSTR_maxwait   , MP_ARG_INT                  , {.u_int  = 0          }},
        { MP_QSTR_interval  , MP_ARG_INT                  , {.u_int  = 500        }},
    };
    ...
    bool is_maxwait = kw[ARG_maxwait].u_int > 0;
    
    if (!self->conn) {
        if (sdcard_waiting(self, kw[ARG_wait].u_bool)) {
            mp_printf(MP_PYTHON_PRINTER, "Waiting For A Card To Be Inserted\n");
            
            if (kw[ARG_maxwait].u_int > 0) {
                while (sdcard_waiting(self, kw[ARG_wait].u_bool) && kw[ARG_maxwait].u_int > 0) {
                    sleep_ms(kw[ARG_interval].u_int);
                    --kw[ARG_maxwait].u_int;
                }
            } else if (!is_maxwait) {
        ...
        if (self->detect == -1 || (self->detect > -1 && gpio_get(self->detect))) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(SDCard_detect_obj, 0, SDCard_detect);

STATIC mp_obj_t SDCard_state(mp_obj_t self_in) {
    sdcard_SDCard_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_printf(MP_PYTHON_PRINTER, "Detected: %b, Connected: %b, Mounted %b\n", (self->detect == -1 || (self->detect > -1 && gpio_get(self->detect))), self->conn, self->mounted);
    return mp_const_none;
}

STATIC MP_DEFINE_CONST_FUN_OBJ_1(SDCard_state_obj, SDCard_state);
...
STATIC mp_obj_t SDCard_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    SDCard_detect(3, setup_args, (mp_map_t *)&mp_const_empty_map);
...
STATIC const mp_rom_map_elem_t SDCard_locals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_detect), MP_ROM_PTR(&SDCard_detect_obj) },
    { MP_ROM_QSTR(MP_QSTR_state), MP_ROM_PTR(&SDCard_state_obj) },
    */
};

STATIC MP_DEFINE_CONST_DICT(SDCard_locals_dict, SDCard_locals_dict_table);

STATIC bool sdcard_warnings(sdcard_SDCard_obj_t *self) {
    if (!self->conn) {
        mp_printf(MP_PYTHON_PRINTER, "SD Card Not Initialized");
        return true;
    }
    
    if (!(self->detect == -1 || (self->detect > -1 && gpio_get(self->detect)))) {
        mp_printf(MP_PYTHON_PRINTER, "SD Card Not Detected");
        return true;
    }
...
STATIC void SDCard_attr(mp_obj_t self_in, qstr attr, mp_obj_t *dest) {
    ...
    if (dest[0] == MP_OBJ_NULL) {
        if (attr == MP_QSTR_drive)
            ...
            dest[0] = self->mounted ? mp_const_true : mp_const_false;
        else if (attr == MP_QSTR_detected)
            dest[0] = ((self->detect == -1 || (self->detect > -1 && gpio_get(self->detect)))) ? mp_const_true : mp_const_false;
        else if (attr == MP_QSTR_mount) {
            if (sdcard_warnings(self)) 
                return;
        ...
        else if (attr == MP_QSTR_detect) {
            dest[0] = MP_OBJ_FROM_PTR(&SDCard_detect_obj);
            dest[1] = self;  
        }
        else if (attr == MP_QSTR_state) {
            dest[0] = MP_OBJ_FROM_PTR(&SDCard_state_obj);
            dest[1] = self;  
        }
    }
}
